class IMV2Decoder {
  ...
  function frameBuffer(bleDelegate, transmittedFrame) {
    try {
      if (bleDelegate.queue.lastPacketType.equals("live") &&
        ...
        var pwm = transmittedFrame
                      .decodeNumber(Lang.NUMBER_FORMAT_SINT16, {
                        : offset => 13, : endianness => Lang.ENDIAN_LITTLE,
                      })
                      .abs() /
                  100.0;
        if (pwm < 100.0) {
      ...
      if (bleDelegate.queue.lastPacketType.equals("stats") &&
          transmittedFrame.size() == 20) {
      ...
      if (bleDelegate.queue.lastPacketType.equals("batStats")) {
        ...
        eucData.batteryTemp2 =
            transmittedFrame.decodeNumber(Lang.NUMBER_FORMAT_SINT8, {
              : offset => 17, : endianness => Lang.ENDIAN_LITTLE,
            }) +
            80.0;  // data[12]
      }
    } catch (e) {
